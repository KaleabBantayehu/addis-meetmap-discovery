...
function makeIcon(label: string, active: boolean) {
  return L.divIcon({
    className: "",
    html: `<div class="mm-pin" style="${active ? "transform:rotate(-45deg) scale(1.15);box-shadow:0 6px 20px rgba(0,0,0,.35);" : ""}"><span>${label}</span></div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 32],
  });
}
    ...
    >
      ...
      {places.map((p) => (
        <Marker
          ...
          eventHandlers={{
            click: () => navigate({ to: "/place/$id", params: { id: p.id } }),
          }}
        />
      ))}
    </MapContainer>
